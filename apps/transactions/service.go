...
func (s service) CreateTransaction(ctx context.Context, req CreateTransactionRequestPayload) (err error) {
	...
	// create transaction
	err = s.repo.CreateTransactionWithTx(ctx, tx, trx)
	if err != nil {
		return
	}
	// update current stock in myProduct
	err = myProduct.UpdateStockProduct(trx.Amount)
	if err != nil {
		return
	}
	// update product into database
	err = s.repo.UpdateProductStockWithTx(ctx, tx, myProduct)
	if err != nil {
		return
	}
